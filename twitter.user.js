...
//#region src/index.ts
	...
	async function createUI() {
		{
			...
			for (const dom of doms) {
				...
				if ((_dom$textContent = dom.textContent) === null || _dom$textContent === void 0 ? void 0 : _dom$textContent.match(/x corp/i)) {
					const switchDom = dom.parentNode.previousSibling.previousSibling.cloneNode(true);
	...
	new MutationObserver(createUI).observe(document.body, {
		childList: true,
		subtree: true
	});
	...
	function handleLike(e) {
		...
		if (!likeDom) return;
		const tweet = queryParent(likeDom, "[role=\"dialog\"]>div>div,[data-testid=\"tweet\"]");
		if (!tweet) return;
		const photos = Array.from(tweet.querySelectorAll("[data-testid=\"tweetPhoto\"],[data-testid=\"swipe-to-dismiss\"]"));
		...
		let href = (_queryParent = queryParent(photos[0], "a")) === null || _queryParent === void 0 ? void 0 : _queryParent.href;
	...
	async function makeWaterMark() {
		...
		ctx.shadowBlur = 4;
		ctx.fillText(author, canvas.width - ctx.measureText(author).width - 16, canvas.height - 16);
		ctx.restore();
		ctx.save();
	...
	async function updateClipboard(likeDom) {
		...
		try {
			if (local.getItem("k-x-mode") === "watermark") {
				...
				await retry(() => navigator.clipboard.write([new ClipboardItem({ [blob.type]: blob })]));
			} else {
				let html = cache.map((item) => item.imgs.map((img) => `<img src="${img}" />`).join("")).join("");
				await retry(() => navigator.clipboard.write([new ClipboardItem({ "text/html": new Blob([html], { type: "text/html" }) })]));
			}
			toast(likeDom, "已复制");
		} catch (err) {
			console.error(err);
