...
//#region src/views/s.ts
	...
	function setupInfiniteScroll() {
		function getAspectRatioFromImg(img) {
			if (!img) return null;
		...
		function getMostCommonAspectRatio() {
			const images = document.querySelectorAll("#i3 .auto-load-img");
			const ratioCountMap = {};
			images.forEach((img) => {
		...
		function bindImgLoadState(img, container) {
			...
			const setPlaceholderText = (text) => {
				const placeholderText = container.querySelector(".auto-load-placeholder-text");
			...
			const onDone = () => {
				...
				if (img.complete && img.naturalWidth > 0) clearLoadingState();
					...
					retryCount++;
					setPlaceholderText(`Load failed, retry #${retryCount}/${MaxRetryCount}...`);
					timer = window.setTimeout(onDone, 6e4);
		...
		document.querySelector("#i1").style.width = "fit-content";
	}
